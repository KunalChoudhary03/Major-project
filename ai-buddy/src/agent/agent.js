...
const toolsCallResults = await Promise.all(toolsCall.map(async(call)=>{
    const tool = tools[call.name];
    if(!tool){
        throw new Error(`Tool ${call.name} not found`);
    }
    const toolInput = call.args
    console.log('Invoking tools:' ,call.name,"with Input:",toolInput);
 ...
 state.messages.push(...toolsCallResults);
return state;
})
